...
def spline1(points):
    ...
    # 上記の連立一次方程式を解く。結果は長さ 2n の配列
    params = np.linalg.solve(A, b)

    # 得られたパラメータに従って補間曲線の座標(x,y)を求める。
    x = []
    y = []
    xp = np.arange(-1,1,0.01) # 刻み
    for j in range(n):
        aj, bj = params[2 * j : 2 * j + 2]
        xj = points[j][0]
        xj1 = points[j+1][0]
        xs = xp[(xp>=xj) & (xp<xj1)]
        ys = aj + bj * xs
...
### データを生成
# スプライン補間
# x, y = spline1(test_samples)
x, y = spline1(eval_samples)
